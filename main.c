...
int main(void) {
   ...
   EXTI15_Init();                       //使能定时器捕获
   OV7670_Window_Set(10, 174, 240, 320); //设置相机窗口
   OV7670_CS = 0;                       //片选
...
INIT:
   ...
   while (!option) {
      tp_dev.scan(0);
      if (tp_dev.sta & TP_PRES_DOWN) { //主菜单, 选择相机或者相册
         BACK_COLOR = LIGHTBLUE;
         if (tp_dev.x[0] <= 170 && tp_dev.x[0] >= 70 && tp_dev.y[0] <= 140 &&
             tp_dev.y[0] >= 80) {
            LCD_Fill(70, 80, 170, 140, LIGHTBLUE);
            LCD_ShowString(100, 100, 100, 16, 16, "Album");
            option = 1;

         } else if (tp_dev.x[0] <= 170 && tp_dev.x[0] >= 70 &&
                    tp_dev.y[0] <= 240 && tp_dev.y[0] >= 180) {
            option = 2;
   ...
   if (option == 2) {
      ...
      picindextbl =
         mymalloc(2 * totpicnum); //申请2*totpicnum个字节的内存,用于存放图片索引
      while (picfileinfo.lfname == NULL || pname == NULL ||
             picindextbl == NULL) //内存分配出错
      {
         LCD_Clear(WHITE); //清除显示
      ...
      if (res == FR_OK) {
         ...
         while (1)  //全部查询一遍
         {
            ...
            if (res != FR_OK || picfileinfo.fname[0] == 0)
               break; //错误了/到末尾了,退出
            fn = (u8 *)(*picfileinfo.lfname ? picfileinfo.lfname
                        : picfileinfo.fname);
            res = f_typetell(fn);
      ...
      piclib_init();                                    //初始化画图
      curindex = 0;                                     //从0开始显示
      res = f_opendir(&picdir, (const TCHAR *)"0:/DCIM"); //打开目录
